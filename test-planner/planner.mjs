...
const buildRequestedSurfaces = (request, env) => {
    ...
    const surfaces = [];
    const skipDefaultRuns = env.AGENTOS_TEST_SKIP_DEFAULT === "1";
    if (!skipDefaultRuns) {
        surfaces.push("unit");
    }
    if (env.AGENTOS_TEST_INCLUDE_EXTENSIONS === "1") {
        surfaces.push("extensions");
    }
    if (env.AGENTOS_TEST_INCLUDE_CHANNELS === "1") {
        surfaces.push("channels");
    }
    if (env.AGENTOS_TEST_INCLUDE_CONTRACTS === "1") {
        surfaces.push("contracts");
    }
    if (env.AGENTOS_TEST_INCLUDE_GATEWAY === "1") {
...
const resolveCIManifestScope = (scope = {}, env = process.env) => ({
    ...
    docsOnly: parseBooleanLike(scope.docsOnly ?? env.AGENTOS_CI_DOCS_ONLY, false),
    docsChanged: parseBooleanLike(scope.docsChanged ?? env.AGENTOS_CI_DOCS_CHANGED, false),
    runNode: parseBooleanLike(scope.runNode ?? env.AGENTOS_CI_RUN_NODE, true),
    runMacos: parseBooleanLike(scope.runMacos ?? env.AGENTOS_CI_RUN_MACOS, true),
    runAndroid: parseBooleanLike(scope.runAndroid ?? env.AGENTOS_CI_RUN_ANDROID, true),
    runWindows: parseBooleanLike(scope.runWindows ?? env.AGENTOS_CI_RUN_WINDOWS, true),
    runSkillsPython: parseBooleanLike(
        scope.runSkillsPython ?? env.AGENTOS_CI_RUN_SKILLS_PYTHON,
        true,
    ),
    hasChangedExtensions: parseBooleanLike(
        scope.hasChangedExtensions ?? env.AGENTOS_CI_HAS_CHANGED_EXTENSIONS,
        false,
    ),
    changedExtensionsMatrix: parseChangedExtensionsMatrix(
        scope.changedExtensionsMatrix ?? env.AGENTOS_CI_CHANGED_EXTENSIONS_MATRIX,
    ),
    runChangedSmoke: parseBooleanLike(
        scope.runChangedSmoke ?? env.AGENTOS_CI_RUN_CHANGED_SMOKE,
        true,
    ),
});
...
const resolveMaxWorkersForUnit = (unit, context) => {
    const overrideWorkers = Number.parseInt(context.env.AGENTOS_TEST_WORKERS ?? "", 10);
...
const withIncludeFileEnv = (context, unitId, files) => ({
    AGENTOS_VITEST_INCLUDE_FILE: context.writeTempJsonArtifact(unitId, files),
});
...
const resolveUnitHeavyFileGroups = (context) => {
    ...
    const heavyUnitFileLimit = parseEnvNumber(
        env,
        "AGENTOS_TEST_HEAVY_UNIT_FILE_LIMIT",
...
const resolveExtensionHeavyFileGroups = (context) => {
    ...
    const memoryHeavyFiles = selectMemoryHeavyFiles({
        ...
        limit: parseEnvNumber(
            env,
            "AGENTOS_TEST_MEMORY_HEAVY_EXTENSION_FILE_LIMIT",
            runtime.isCI ? 12 : 6,
        ),
        minDeltaKb: parseEnvNumber(
            env,
            "AGENTOS_TEST_MEMORY_HEAVY_EXTENSION_MIN_KB",
    ...
    const timedHeavyFiles = selectTimedHeavyFiles({
        candidates: extensionCandidates,
        limit: parseEnvNumber(env, "AGENTOS_TEST_HEAVY_EXTENSION_FILE_LIMIT", runtime.isCI ? 16 : 8),
        minDurationMs: parseEnvNumber(
            env,
            "AGENTOS_TEST_HEAVY_EXTENSION_MIN_MS",
...
const resolveChannelTimedHeavyFiles = (context) => {
    ...
    const timedHeavyChannelFileLimit = parseEnvNumber(
        env,
        "AGENTOS_TEST_HEAVY_CHANNEL_FILE_LIMIT",
        runtime.isCI ? 10 : 6,
    );
    const timedHeavyChannelMinDurationMs = parseEnvNumber(
        env,
        "AGENTOS_TEST_HEAVY_CHANNEL_MIN_MS",
...
const buildDefaultUnits = (context, request) => {
    ...
    const extensionsBatchTargetMs = parseEnvNumber(
        env,
        "AGENTOS_TEST_EXTENSIONS_BATCH_TARGET_MS",
    ...
    const unitFastLaneCount = Math.max(
        1,
        parseEnvNumber(env, "AGENTOS_TEST_UNIT_FAST_LANES", defaultUnitFastLaneCount),
    );
    ...
    const unitFastBatchTargetMs = parseEnvNumber(
        env,
        "AGENTOS_TEST_UNIT_FAST_BATCH_TARGET_MS",
    ...
    const channelsBatchTargetMs = parseEnvNumber(
        env,
        "AGENTOS_TEST_CHANNELS_BATCH_TARGET_MS",
...
const buildTargetedUnits = (context, request) => {
    ...
    const targetedUnitBatchTargetMs = parseEnvNumber(
        context.env,
        "AGENTOS_TEST_TARGETED_UNIT_BATCH_TARGET_MS",
    ...
    const targetedChannelsBatchTargetMs = parseEnvNumber(
        context.env,
        "AGENTOS_TEST_TARGETED_CHANNELS_BATCH_TARGET_MS",
    ...
    const groups = request.fileFilters.reduce((acc, fileFilter) => {
        ...
        if (matchedFiles.length === 0) {
            ...
            const key = `${classification.legacyBasePinned ? "base-pinned" : classification.surface}:${classification.isolated ? "isolated" : "default"
                }`;
    ...
    const nextUnit = createExecutionUnit(context, {
        ...
        env:
            Array.isArray(unit.includeFiles) && unit.includeFiles.length > 0
                ? {
                    ...unit.env,
                    AGENTOS_VITEST_INCLUDE_FILE: context.writeTempJsonArtifact(
                ...
                ? [
                    {
                        ...
                        command: [
                            "pnpm build",
                            "pnpm ui:build",
                            "node agentos.mjs --help",
                            "node agentos.mjs status --json --timeout 1",
...
export function explainExecutionTarget(request, options = {}) {
    ...
    context.noIsolateArgs =
        context.env.AGENTOS_TEST_ISOLATE === "1" || context.env.AGENTOS_TEST_ISOLATE === "true"
            ? []
            : context.env.AGENTOS_TEST_NO_ISOLATE !== "0" &&
                context.env.AGENTOS_TEST_NO_ISOLATE !== "false"
                ? ["--isolate=false"]
                : [];
...
export function buildExecutionPlan(request, options = {}) {
    ...
    context.noIsolateArgs =
        env.AGENTOS_TEST_ISOLATE === "1" || env.AGENTOS_TEST_ISOLATE === "true"
            ? []
            : env.AGENTOS_TEST_NO_ISOLATE !== "0" && env.AGENTOS_TEST_NO_ISOLATE !== "false"
                ? ["--isolate=false"]
                : [];
    ...
    const shardOverride = Number.parseInt(env.AGENTOS_TEST_SHARDS ?? "", 10);
    ...
    const shardIndexOverride = Number.parseInt(env.AGENTOS_TEST_SHARD_INDEX ?? "", 10);
    context.shardIndexOverride =
        Number.isFinite(shardIndexOverride) && shardIndexOverride > 0 ? shardIndexOverride : null;

    if (context.shardIndexOverride !== null && context.shardCount <= 1) {
        throw new Error(
            `AGENTOS_TEST_SHARD_INDEX=${String(context.shardIndexOverride)} requires AGENTOS_TEST_SHARDS>1.`,
        );
    }
    if (context.shardIndexOverride !== null && context.shardIndexOverride > context.shardCount) {
        throw new Error(
            `AGENTOS_TEST_SHARD_INDEX=${String(context.shardIndexOverride)} exceeds AGENTOS_TEST_SHARDS=${String(context.shardCount)}.`,
        );
    }
    ...
    const topLevelParallelLimit = Math.max(
        1,
        parseEnvNumber(env, "AGENTOS_TEST_TOP_LEVEL_CONCURRENCY", defaultTopLevelParallelLimit),
    );

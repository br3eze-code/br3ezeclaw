...
const resolveLoadRatio = (env, cpuCount, platform, loadAverage) => {
    const loadAwareDisabledRaw = env.AGENTOS_TEST_LOAD_AWARE?.trim().toLowerCase();
...
export function resolveRuntimeCapabilities(env = process.env, options = {}) {
    ...
    const hostCpuCount =
        parsePositiveInt(env.AGENTOS_TEST_HOST_CPU_COUNT) ?? options.cpuCount ?? os.cpus().length;
    const totalMemoryBytes = options.totalMemoryBytes ?? os.totalmem();
    const hostMemoryGiB =
        parsePositiveInt(env.AGENTOS_TEST_HOST_MEMORY_GIB) ?? Math.floor(totalMemoryBytes / 1024 ** 3);
    const nodeMajor = Number.parseInt(
        (options.nodeVersion ?? process.versions.node).split(".")[0] ?? "",
        10,
    );
    const intentProfile = parseProfile(options.profile ?? env.AGENTOS_TEST_PROFILE ?? "normal");
...
export function resolveLocalVitestMaxWorkers(env = process.env, options = {}) {
    const explicit = parsePositiveInt(env.AGENTOS_MAX_WORKERS);
    if (explicit !== null) {
        return explicit;
    }
